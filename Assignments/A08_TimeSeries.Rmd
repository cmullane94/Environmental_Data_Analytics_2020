...
for(i in 1:length(Ozone_files)) {
  Ozone_files_named[[i]] <- assign(paste("GaringerOzone201", i-1, sep = ""), read.csv(Ozone_files[i]))
}
...
#Combining datasets into one using rbind_list; rbind_list was used so the dataframes could be bound from the list created above. To use just rbind, the code would have looked like this:
...
Days <- rename(Days, "Date" = "seq(from = as.Date(\"2010-01-01\"), to = as.Date(\"2019-12-31\"), by = 1)")
...
ggplot(data = GaringerOzone, 
  ...
  geom_line() +
  labs(y = "Max 8 hour Ozone Concontration (ppm)")
...
> Answer: Piecewise constant interpolation would fill missing data values with previous or subsequent measurements. Because this dataset appears to be highly variable in spots, using measurements that are the same as the adjacent data points may be inaccurate. A spline interpolation would use a quadratic or other low-degree polynomial equation to populate missing values. However, we are not using non-linear functions to describe or depict our data currently; existing data points are connected using linear functions. Attempting to fill data with spline interpolation would result in data points that are smoother than the dataset we currently have. 
...
> Answer: The graph for Question 7 exhibits clear seasonal trends; increasing and decreasing ozone concentration values repeat over time. The seasonal Mann-Kendall time series analysis is the only test that accounts for seasonality when analyzing monotonic trends. Additionally, our data are nonparametric, i.e., they do not come from a population that has a probability distribution with fixed parameters (such as a normal distribution). Additionally, because we are not using daily data and instead found the mean ozone concentration by month, we do not have to account for temporal autocorrelation. This factors eliminate the linear regression, Mann-Kendall, and modified Mann-Kendall tests as possibilities and make the seasonal Mann-Kendall the best option.
...
> Answer: While there are no significant seasonal (i.e., monthly) trends in mean ozone concentration from 2010 to 2019, mean ozone concentration significantly decreased overall during the 2010s (z = -1.963, p < 0.05). When quantified as a linear regression, this trend has a slope of -0.0002 -- that is, mean ozone concentration decreases by 0.0002 ppm with each passing month.
