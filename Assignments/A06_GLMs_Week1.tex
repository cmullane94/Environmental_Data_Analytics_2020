...
\CommentTok{#2}
\CommentTok{#Looking at column names and date format}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#For number 5: run lm, aov, and then stop because nothing is significant. We chose to do a 2-way anova with interactions, so we have to stop after that without any post-hoc tests.}

\CommentTok{#5 Test for tn}
\CommentTok{##As linear model}
\NormalTok{tn.anova}\FloatTok{.2}\NormalTok{way.int <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Peter_Paul_Nutrients_wr, tn_ug }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\KeywordTok{as.factor}\NormalTok{(month))}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{###No significant interactions; no need to form groups}

\CommentTok{#6 Test for tp}
\CommentTok{##As linear model}
\NormalTok{tp.anova}\FloatTok{.2}\NormalTok{way.int <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Peter_Paul_Nutrients_wr, tp_ug }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\KeywordTok{as.factor}\NormalTok{(month))}
...
\NormalTok{tn.anova.plot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(Peter_Paul_Nutrients_wr, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ tn_ug, }\DataTypeTok{x =} \KeywordTok{as.factor}\NormalTok{(month), }\DataTypeTok{fill =}\NormalTok{ lakename)) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{scale_fill_manual}\NormalTok{(}\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"#66c2a5"}\NormalTok{, }\StringTok{"#fc8d62"}\NormalTok{))}
...
\NormalTok{tp.anova.plot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(Peter_Paul_Nutrients_wr, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ tp_ug, }\DataTypeTok{x =} \KeywordTok{as.factor}\NormalTok{(month), }\DataTypeTok{fill =}\NormalTok{ lakename)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{() }\OperatorTok{+}\StringTok{ }
...
\NormalTok{tp_tn_plot <-}\StringTok{ }\KeywordTok{plot_grid}\NormalTok{(tn.anova.plot }\OperatorTok{+}\StringTok{ }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position=}\StringTok{"none"}\NormalTok{),}
