...
EPA_air <- list(EPAair_PM25_NC2019, EPAair_PM25_NC2018, EPAair_O3_NC2019, EPAair_O3_NC2018)
names(EPA_air) <- c("EPAair_PM25_NC2019", "EPAair_PM25_NC2018", "EPAair_O3_NC2019", "EPAair_O3_NC2018")

lapply(EPA_air, function(x) {dim(x)})
lapply(EPA_air, function(x) {colnames(x)})
invisible(lapply(EPA_air, function(x) {str(x)}))
...
sapply(names(EPA_subset), function (x) {write.csv(EPA_subset[x], row.names = FALSE, file = paste("./Data/Processed/", x, "_processed.csv", sep = ""))})
...
```{r}
#7
EPA_combined <- rbind(EPA_subset[[1]], EPA_subset[[2]], EPA_subset[[3]], EPA_subset[[4]])
