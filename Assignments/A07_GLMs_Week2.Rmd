...
deftheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "right", panel.grid = element_line(color = "black"))
theme_set(deftheme)
...
> Answer: Yes, there is a significant interaction between depth and lakename. A model that uses the interaction effects between depth and lakename to predict temperature explains 78.57% of the observed variance in temperature.
...
#Marginal rsquared: fixed effects
#Conditional rquared: rsquared when you add in the random effects
...
> Answer: About 2% more is explained by adding the random effect to the model. 
...
> Answer: The mixed effects model is better than the fixed effects model; the AIC is lower by a value of 20. The p value for the comparison of the two models is <.0001, indicating that this difference results in a significantly different fit (and that the model with the lower AIC is significantly better).