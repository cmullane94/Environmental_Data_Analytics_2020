...
***The sp package is the older version of the sf package
...
Start by reading in the basin boundaries for non-reference (i.e., notable human impact) for the Southeast Plains using `st_read()` from the sf package. Notice how you only call the .shp file here. You don't need to include all the other files that come along with the .shp file (i.e., .prj, .sbn, .shx, .sbx, .dbf, etc.). ***These other files still need to be present
...
colnames(basins_nf_seplains_raw)
#3
length(colnames(basins_nf_seplains_raw))
dim(basins_nf_seplains_raw)
#4
...
st_crs(southeast_state_bounds) <- my_epsg
...
# set the proj4 string first and then set the EPSG code (because proj4 will over-ride EPSG if you set EPSG first)
southeast_state_bounds_na_albers <- st_transform(x = southeast_state_bounds, crs = na_albers_proj4)
st_crs(southeast_state_bounds_na_albers) <- na_albers_epsg
...
For example, you might want to select a state from southeast_state_bounds and find its area. You might want to find how many gages are inside that state.

#### 3.1.1 A Note on Spatial Geometry

In some cases, we might not necessarily need the tabular data associated with a spatial dataset. For example, say you decide that you just want to look at GAGES-II data in North Carolina. Once you have the boundary lines for North Carolina you might not need all the spatial data associated with it. You can remove the tabular data using the st_geometry() command. Sometimes it helps to speed up plotting if you don't have the tabular data for a big spatial dataset.
...
# add your code here
#Selects only the geometry
head(southeast_state_bounds)
southeast_state_bounds_NC<- southeast_state_bounds %>%
  filter(NAME == "North Carolina") %>%
  st_geometry()

# check
# add your code here
head(southeast_state_bounds_NC)

# select watersheds that intersect with NC bounds
# add your code here
nc_basins_nf_seplains <- st_intersection(basins_nf_seplains, southeast_state_bounds_NC)

#As a pipe
nc_basins_nf_seplains <- basins_nf_seplains %>%
  st_intersection(southeast_state_bounds_NC)
...
# add your code here
nc_gages <- st_intersection(basins_nf_seplains, gages)
...
ggplot(data = southeast_state_bounds) + # moved this up here so I don't have to keep calling it
  geom_sf() +
...
ggplot(data = nc_basins_nf_seplains) + 
  geom_sf() +
  theme_classic()
dev.off()
...
nc_bbox <- nc_state_bounds_geom %>%
  st_buffer(dist = 170000) %>% # this value is very arbitrary just wanting to make a large buffer around geometry
  st_transform(4326) %>% # WGS84 (for lat and long)
  st_bbox() #Returns the bounding of a simple feature/simple feature set
nc_bbox # check
...
nc_basemap <- get_map(nc_bbox_fix, maptype = 'terrain-background', source = 'stamen', zoom = 7)
class(nc_basemap)
st_crs(nc_basemap)
...
# bonus
# try changing the zoom on get_map() and see what happens
...
***1b was skipped in the answer key
...
nc_basins_nf_seplains_overlap <- st_overlaps(southeast_state_bounds_NC, basins_nf_seplains, sparse = FALSE, prepared = TRUE)

# look at result
# add your code here
head(nc_basins_nf_seplains_overlap)

# make a function to convert binary to logic
# add your code here
##[Done within st_overlaps]

# map function on data and select only true values
# add your code here
nc_basins_overlap_mutated <- nc_basins_nf_seplains_overlap %>%
  t() %>%
  as.data.frame() %>%
  mutate(Value = 1:1232)

basins_mutated <- mutate(basins_nf_seplains, Value = 1:1232)
  
nc_basins_overlap <- full_join(nc_basins_overlap_mutated, basins_mutated)

# look at result
# add your code here
head(nc_basins_overlap)

# use nc_basins_overlap_logic to select only watersheds that overlap
# add your code here
nc_basins_overlap_logic <- filter(nc_basins_overlap, V1 == "TRUE")
nc_basins_overlap_logic <- st_as_sf(nc_basins_overlap_logic, sf_column_name = "geometry")

# check class
# add your code here
class(nc_basins_overlap_logic)

# select Southeast gages only
# add your code here
# stretch break...this might take a while to run
##Not doing this...found using the whole gages data set was easier

# use nc_basins_overlap_sel to find gages inside
# add your code here
# stretch break...this might take a while to run

#Merging all watershed polygons into one
COMBINED_nc_basins_overlap_logic <- st_union(nc_basins_overlap_logic)

#Checking and transforming coordinate systems
st_crs(COMBINED_nc_basins_overlap_logic)
st_crs(gages)

gages_4326 <- gages %>%
  st_transform(my_epsg_2)

#COMBINED_nc_basins_overlap_logic <- st_buffer(COMBINED_nc_basins_overlap_logic, 0)

#gages_southeast <- st_buffer(gages_southeast, 0)

#Checking validity of datasets
st_is_valid(gages)
st_is_valid(COMBINED_nc_basins_overlap_logic)

nc_gages_intersect <- st_intersection(gages_4326, COMBINED_nc_basins_overlap_logic)

#nc_gages_overlap_sel_NA <- na_if(nc_gages_overlap_sel[1:998], "FALSE")

#nc_gages_overlap_mutated <- nc_gages_overlap_sel %>%
  #t() %>%
  #as.data.frame() %>%
  #gather()  %>%
  #mutate(Value = 1:998)
  
#gages_southeast_mutated <- mutate(gages_southeast, Value = 1:998)
  
#nc_gages_overlap <- full_join(nc_gages_overlap_mutated, gages_southeast_mutated)
...
nc_gages_intersect <- nc_gages_intersect %>%
  st_transform(my_epsg_2)
st_crs(nc_gages_intersect)

nc_basins_overlap_logic <- nc_basins_overlap_logic %>%
  st_transform(my_epsg_2)
st_crs(nc_basins_overlap_logic)

# plot figure 1b
# add your code here
pdf(here("outputs", "Mullaney_A09_Spatial.pdf"), width = 11, height = 8.5)
# add your code here
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, color = "black", 
          lwd = 1, fill = NA, inherit.aes = FALSE) +
  geom_sf(data = nc_basins_overlap_logic, color = "black", 
          alpha = 0.25, inherit.aes = FALSE) +
  geom_sf(data = nc_gages_intersect_2, inherit.aes = FALSE) +
  labs(x = "Longitude", y = "Latitude")
dev.off()
```

```{r miscellaneous plots}
###[[[Trying to see what the original watersheds and states look like -- it worked, just took awhile]]
pdf(here("outputs", "merged_watersheds_test.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = COMBINED_nc_basins_overlap_logic)
  dev.off()
  
pdf(here("outputs", "se_gages_test.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = gages_southeast)
  dev.off()

pdf(here("outputs", "nc_watersheds_only_test.pdf"), width = 11, height = 8.5)
ggplot(data = basins_nf_seplains_geom) +
  geom_sf() +
  dev.off()
...
pdf(here("outputs", "nc_watersheds_states_test.pdf"), width = 11, height = 8.5)
ggplot(southeast_state_bounds, aes()) +
  geom_sf(color = "black") +
  geom_sf(data = basins_nf_seplains_geom, color = "blue", alpha = 0.25) +
  theme_bw()
  dev.off()
  
#Example from using ggmap above
pdf(here("outputs", "nc_with_basemap.pdf"), width = 11, height = 8.5)
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE)
dev.off()

pdf(here("outputs", "nc_basins_overlap.pdf"), width = 11, height = 8.5)
ggplot(data = nc_basins_overlap_logic) +
  geom_sf()
  dev.off()
...
# In this example, we choose the climate data and use the WD_SITE variable. This is the "Site average of annual number of days (days) of measurable precipitation, derived from 30 years of record (1961-1990), 2km PRISM.". This description comes from the gagesII_sept30_2011_var_desc.xlsx file in the tabular data directory.
...
#Changing them both to factors causes NAs in joined data and a warning message; use numeric classes

nc_gages$STAID <- as.numeric(nc_gages$STAID)
my_tabular_data_raw$STAID <- as.numeric(my_tabular_data_raw$STAID)
...
names(nc_basins_nf_seplains) 

nc_basins_nf_seplains <- rename(nc_basins_nf_seplains, STAID = GAGE_ID)
nc_basins_nf_seplains$STAID <- as.numeric(nc_basins_nf_seplains$STAID)

#Could do in one line of code:

nc_basins_overlap_sel_fix <- nc_basins_overlap_sel %>%
  mutate(STAID = as.character(GAGE_ID)) 

# join tabular data
# add your code here

nc_watersheds_join <- nc_basins_nf_seplains %>%
  left_join(my_tabular_data_raw, by = "STAID")

# check result
# add your code here

names(nc_watersheds_join)

# plot
pdf(here("outputs", "nc_basins_by_tabular_data.pdf"), width = 11, height = 8.5)
# add your code here
ggplot(data = nc_watersheds_join, aes(fill = WD_BASIN)) +
  geom_sf() +
  geom_sf(data = nc_state_bounds_geom, fill = NA) +
...
Questions:
Do we need other map elements?
Overlapping?

```{r assignment 9, option 1}
pdf(here("outputs", "Mullaney_A09_Spatial_Option_1.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = nc_watersheds_join, aes(fill = PPTAVG_BASIN), color = "black", alpha = 0.5) +
  geom_sf(data = nc_state_bounds_geom, fill = NA, 
          color = "black") +
  geom_sf(data = nc_gages) +
  scale_fill_viridis_c(option = "plasma", direction = -1) +
  #scale_fill_gradient(low = "white", high = "blue") +
  labs(fill = "Mean annual precipitation (cm)") +
  theme_bw()
dev.off()
```
...
# add your code here
st_write(nc_watersheds_join, here("outputs", "nc_watersheds.shp"))
```
...
This package is nice package for making maps. I didn't have enough time to cover it but I recommend checking it out. More information [here](https://github.com/mtennekes/tmap).
...
Some good places to start looking for help with the sf package include the [rspatial website](https://cengel.github.io/R-spatial/) and the [sf package vignettes](https://r-spatial.github.io/sf/articles/sf1.html). We've listed out some other reference materials too.
...
* [Ecological Models and Data in R by Bolker - Book](https://www.amazon.com/Ecological-Models-Data-Benjamin-Bolker/dp/0691125228)
...
We've included a copy of the sf package cheatsheet in the project folder. I'd recommend printing it off and keeping it close-by for easy reference. You can find a number of cheatsheets for other tidyverse packages [here](https://rstudio.com/resources/cheatsheets/).
