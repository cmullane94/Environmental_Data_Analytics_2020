...
library(tidyverse)
library(lubridate)
#When working with miultiple similar packages: identical functions in one package can mask those in another -- i.e., if you call a function, it will reference the function in one package over those in another. Make sure you load the packages in the correct order.
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4) #The names of the existing columns will go in the nutrient column; the former values in those columns will be gathered into one concentration column
...
# Spread nutrient data into separate columns; should get us close to being back the dimensions of the original dataset
...
#Will not preserve the initial colum
...
LitterFiles
#Each file name has been stored as a value
#If the files don't have the same column names, you will have to use a join function

Litter <- LitterFiles %>%
  ldply(read.csv) #Telling it to read anything that is a CSV in LitterFiles
```
...
#Will often give warning messages; don't always mean something is wrong
#We are telling it to join all of the columns from Trap except for the ones that we are telling it to join by (siteID, plotID, and trapID will be merged)
#If there are other columns that are identical across both and those columns are not specified in the join, both columns will be preserved (one will be called x and the other y)
dim(Litter)
dim(Trap)
dim(LitterTrap)
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#Taking out complete NAs and adding new columns by performing mathmatical operations on existing ones
