...
getwd()
library(tidyverse)
#comment out install packages lines in r markdown document
...
`==` Exactly equal to; can be used with factors or number
`!=` Not equal to
`<` Less than
...
# matrix filtering
#Only filtering by rows, not columns = comma followed by nothing
...
#Filtering by rows that have Peter or Paul lake
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
#SFiltering by all lakes that are not Peter and Paul lake
...
#Comma is the same as `&` in this case
...
NTL.phys.data.Tues.1990thru1999 <- filter(NTL.phys.data, 
                                          lakename == "Tuesday Lake" & 
                                            year4 >= 1990 & year4 <= 1999)
head(NTL.phys.data.1999)
dim(NTL.phys.data.1999)

head(NTL.phys.data.Tues.1990thru1999)
dim(NTL.phys.data.Tues.1990thru1999)
```
Question: Why don't we filter using row numbers?

> Answer: The code will not be viable if any data is added or removed.
...
```{r}
#Ascending depth (do not have to specify because it is the default)
...
#Shallow depths; June and July; East, West, and Central Long Lakes, Hummingbird Lake
...
```{r}
#Include the columns lakename and sampledate through temp
#To exclude columns, put a minus sign in front of the column name
#Within one select function, you cannot mix including and excluding
...
#To rearrange: could use the select function to call the columns in the order you want them to appear (see code in below chunk)
...
```{r}
# add a month column to the dataset by asking r to pull a month out of an existing date
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% #This symbol designates a pipe; if thinking about it in terms of a verb, the pipe means "then"
