...
```{r}
#Why do you need pipes here?
#drop_na - can select specific columns of daata
#na.omit and na.exclude — any row that has an NA in any column will be taken out
...
#Time is over 35 years
#Seasonal + Trend + Remainder
#Bars on the side are scale bars 
#Trend and seasonal show a very small portion of the data (based on the scale bars), so the remainder looks very similar to the data
#Seasonal -- what is the overall seasonal effect thtat can be observed year to year 
plot(EnoDischarge.late_Decomposed)

# We can extract the components and turn them into data frames
#Seasonal, trend, and data components of the time series are in columns 1-3, even though you can't see them
...
#stl function does not obey data boundaries
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = seasonal, x = Date), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) + #dashed line at 0
  ylab(expression("Discharge (ft"^3*"/s)"))
...
* **t-test (paired or unpaired)** #assume normal distribution & equal variance across the datasets
...
#Ratio is significantly different from one -- unequal variance

#Violation of both assumptions of a t test
...
#True location shift is not equal to zero
```

How might you interpret the results of this test, and how might you represent them graphically?

>Can conclude that the mean of the early period is significantly higher than the mean of the late period.
"Mean daily discharge in the Eno Rver from 1927-1971 was significantly higher than mean daily discharge from 1985-2019 (Wilcoxon text, W = 1.17*10^8, p <0.001)
Could graph in a box plot, density plot (probability density..?), violin plot

***End of class 2/25/2020***
...
> aggregation: Combing data to use a larger time scale -- e.g., bundling months to make years


> subsampling: selecting portions of the data to use

> interpolation: Filling in gaps via a specific method
...
Highly variable -- one point from each isn't necessarily representative?
...
Would not need to report S values, but good to think about; would report Z statistic and P value (must report both; P does not mean anything outside of the text statistic)

> There is no significat seasonal trend in discharge data for the Eno river from 1985 - 2019 (z = -0.13967, p > 0.05).
