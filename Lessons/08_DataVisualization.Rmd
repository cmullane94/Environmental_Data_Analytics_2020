...
#Corrected -- filling in the points should be in the geom_point layer
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
...
#Color should be in the aesthetic layer here
#Is color only in the aesthetic layer when you are using a variable in the dataset to specify it?
PMplot <- 
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #A new graph will be created for each site
#nrow indicates how many rows the graphs will be broken into
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #facet_grid allows us to facet across two different variables; order matters here; the first variable indicates the rows, the second indicates columns
...
#Points connected by lines (adding geom_point to the code above)
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line() +
...
Surf_Temp_Year <- ggplot(subset(PeterPaul.chem.nutrients, depth == 0), 
       aes(x = daynum, y = temperature_C, color = year4)) +
  facet_wrap(vars(lakename), nrow = 2)+
  geom_point() + labs(x = "Day of Year", y = "Temperature (C)") + scale_color_continuous(name = "Year") 

#2. 
# Plot temperature by date. Color your points by depth.
# Change the size of your point to 0.5

Temp_Date <- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, 
                                     y = temperature_C, color = depth)) +
  geom_point(size = 0.5) + 
  labs (x = "Date Sample Taken",y = "Temperature (C)") + scale_color_continuous(name = "Depth")

print(Surf_Temp_Year)
print(Temp_Date)

#Not a true time series -- done at multiple depths and at two lakes; a true time series would track changes at one spot over time (then, you could use a scatter plot)
...
#Contains outliers that we know are not accurate; can rescale instead of adjusting the data frame
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm, se = FALSE) + #can add se = FALSE to get rid of smoothing
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill fills the boxplots themselves in
print(Nutrientplot3)

# Dot plot -- works really well for a few points
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
#Six different columns; must specify each column separately
Nutrientplot6 <-
...
#Don't need to specify each nutrient column because they have all been gathered into one overall column
Nutrientplot7 <-   
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #Alpha is the level of transparency; specified outside of the aesthetic
print(Nutrientplot6)
