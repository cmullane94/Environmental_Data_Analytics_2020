...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #adding horizontal lines at 50 & 100
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #Making points 50% transparent and changing their size
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #Specifying locations for a label based on an x and y value
...
#Same code without comments -- trying to get the graph to show up
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
...
NvsP2 <-
  ...
  labs(x = expression(paste("TP ( ", mu, "g/L)")), y = expression(paste("TN ( ", mu, "g/L)")), 
       color = "Depth (m)", shape = "") + # change your legend labels here; can adjust all plot labels within "labs"
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x = "", y = (expression(paste("Concentration (", mu,
                                     "g/L)")))) +
  color_scale_discrete()+ #Specify legend labels here...why can't we do it the same way as above?
