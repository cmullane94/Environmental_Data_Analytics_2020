...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y-%m-%d") #Replacing a column in the data frame, so it is on both sides of the function
class(USGS.flow.data$datetime)
...
#Specify the number of data points within each bin
ggplot(USGS.flow.data) +
...
#Formatting: each layer you are formatting has a plus sign after it
...
```{r, fig.height = 3, fig.width = 4}
#Comparing gage height mean, max, and min
ggplot(USGS.flow.data) +
...
#violin plot; can scale based on different qualities in the data
ggplot(USGS.flow.data) +
