...
```{r, message = FALSE}
#message = FALSE will prevent messages from 
# Load package
library(tidyverse)
```
...
#The period indicates that r should look one folder in (one folder in from what? The wd?)

# What happens if we don't assign a name to our imported dataset?
#read.csv("../Data/Raw/USGS_Site02085000_Flow_Raw.csv")
#Two periods indicate that r should look out one folder 
...
> ANSWER: There is a consistent naming convention that is explained in detail in the README; direct links to more information were given; contact information for the data assembler was provided; dates for data collection are given
...
#rows, columns
dim(USGS.flow.data)
#length will give the number of columns, not the number of rows
...
> Answer: They are populated for NAs
...
Removing NAs will completely eliminate all rows containing NAS
...
> Answer: Some statistical analyses will only work when no NAs are present; sometimes knowledge of where data has not been collected is useful
...
#Telling r that a column in a data frame is a date
#Format is the current format that the date in the data is in, not the desired format
