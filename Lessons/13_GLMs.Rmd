...
#Intercept: TN in Paul Lake
#Intercapt + First alpha: TN in Peter Lake
#Beta: coefficient of depth regardless of lake; for each meter deeper, TN increases by 68.559

# interaction effects
#depth is only for Paul Lake; look at lakenamePeter Lake:depth for Peter Lake?
...
```{r}
#Use lme with random effects
#Random effects need to be categorical
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, 
                     random = ~1|lakename)
summary(TempTest.mixed)
#Marginal r-squared: fixed effects
#Conditional r-quared: rsquared when you add in the random effects
rsquared(TempTest.mixed)

#Only fixed effects
# Compare the random effects model with the fixed effects model
#GLS guves you slightly different output with different information; could also run as an lm
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d() +
  #drawing your own line; just give it an intercept and a slope. We use the ones from the random effect model here. The random effect itself isn't represented on the graph in any way, I don't think...
  geom_abline(intercept = -97.72, slope = 0.06) +
  #Adding line from fixed effects model -- doing geom smooth "lm" just takes the x and y relationship; do not use this method if you are not absolutely sure this is what you want to represent
...
> ANSWER: With each passing year, the temperature increases by 0.06 degrees C....what else to say about this?
