...
vector1[3] #returns only the third value from vector1 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5, ncol = 4) #this is a function; numbers automatically increase down each column
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #numbers increase across each row
matrix2
...
matrix1[4, ] #returns row four of matrix1
matrix1[ , 3] #returns column three of matrix1
matrix1[c(12, 14)] #returns the values 12 and 14 from matrix1
matrix1[c(12:14)] #returns the values 12-14 from matrix1
matrix1[2:4, 1:3] #returns rows 2-4 and columns 1-3 from matrix1
...
dataframe1 <- data.frame(d,e,f) #Creates a data frame with d, e, and f values
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # 

dataframe1[1:2,] #returns just rows 1 & 2
dataframe1[c("ID","Passed")] #returns just columns ID and Passed
dataframe1$ID#returns just the column ID

```
Question: How do the different types of data appear in the Environment tab?

> Answer: Matrices appear as single lines; lists and data frames appear as drop-down menus showing the elements of the list/frame

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: The line autocompletes with the name of one of the columns
...
high.temp.F <- c(62, 45, 49, 50, 40, 40, 45, 52, 54, 58)
low.temp.F <- c(31, 29, 43, 24, 23, 23, 26, 30, 42, 41)
...
```{r}
high.temp.C <- (high.temp.F - 32)/1.8
low.temp.C <- (low.temp.F - 32)/1.8
```
...
dataframetemp <- data.frame(high.temp.F, low.temp.F, high.temp.C, low.temp.C)
names(dataframetemp) <- c("High Temperatures (F)", "Low Temperatures (F)", "High Temperatures (C)", "Low Temperatures (C)")
...
sd(dataframetemp$`High Temperatures (F)`)
summary(dataframetemp$`High Temperatures (F)`)
```
...
#difference between library vs. require: 
