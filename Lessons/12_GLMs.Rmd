...
library(tidyverse)
options(scipen = 4)
###From the help menion: "integer. A penalty to be applied when deciding to print numeric values in fixed or exponential notation. Positive values bias towards fixed and negative towards scientific notation: fixed notation will be preferred unless it is more than scipen digits wider."
...
>  For each meter of additional depth, irradiance decreases by -95.890 units.
...
```{r, fig.height = 3, fig.width = 4}
#Removing outliers and values equal to zero
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & 
                                     irradianceWater < 5000)

#Taking a closer look at the plots without the outlier (can more easily see that a transformation is needed)
irradiance.regression1 <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth)

par(mfrow = c(2,2))
plot(irradiance.regression1)
...
#Using a log10 scale for the y axis
#So you don't need to plot the transformed variable?
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
...
#Can plot the transformed variable without the scale layer to get the same thing
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = log(irradianceWater))) +
  geom_smooth(method = "lm") +
  #scale_y_log10() +
  geom_point() 
print(irradiancebydepth2)
...
``` {r}
#Null hypothesis -- rho equals zero and there is no association between the two variables
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth, 
...
step(TPAIC)
#Automatically emoves variables until the lowest AIC is obtained
#How does it know which variable to remove?

#Running the best model to obtain F statistics, p values, etc.
